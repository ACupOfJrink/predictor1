...
# Choose target "Death" 
    ...
    resp_rate_median_value1 = st.number_input("resp rate median value:respiratory rate(insp/min)", value=23.55, min_value=11.00, max_value=43.50, key='resp_rate_median_value1')
    sbp_min_value1 = st.number_input("sbp min value: systolic blood pressure(mmHg)", value=90.73, min_value=6.00, max_value=149.00, key='sbp_min_value1')
    agp_avg1 = st.number_input("agp avg1:Anion gap(mEq/L)", value=19.32, min_value=4.00, max_value=29.00, key='agp_avg1')
    hr_max_value1 = st.number_input("hr max value1:Heart rate (bpm)", value=167.86, min_value=66.00, max_value=282.00, key='hr_max_value1')
    icustay_seq1 = st.number_input("icustay seq:ICU sequence (No unit):", value=3.11, min_value=1.00, max_value=5.00, key='icustay_seq1')
    Disease_Onset_Period1 = st.number_input("Disease Onset Period(DOP):Hours between hospital admit and ICU admit(hours)", value=120.68, min_value=0.00, max_value=186.00, key='Disease_Onset_Period1')
    agp_min1 = st.number_input("agp min:Anion gap(mEq/L)", value=17.91, min_value=-10.00, max_value=33.00, key='agp_min1')
    lactate_mean1 = st.number_input("lactate mean (mg)", value=12.50, min_value=0.30, max_value=27.00, key='lactate_mean1')
    Age1 = st.number_input("Age: Lastest record(year)", value=84.78, min_value=18.00, max_value=102.00, key='Age1')
    wbc1 = st.number_input("wbc: Lastest recorded white blood count(K/uL)", value=50.23, min_value=0.10, max_value=92.20, key='wbc1')
    resp_rate_max_value1 = st.number_input("resp rate max value:respiratory rate(insp/min)", value=134.94, min_value=15.00, max_value=536.00, key='resp_rate_max_value1')
    bun1 = st.number_input("bun: The lastest recorded blood urea nitrogen(mg/dL)", value=73.57, min_value=3.00, max_value=180.00, key='bun1')
    hr_min_value1 = st.number_input("HR(Heart Rate) min value (bpm)", value=84.62, min_value=7.00, max_value=149.00, key='hr_min_value1')
    creatinine_baseline1 = st.number_input("creatinine baseline(mg/dL):", value=3.04, min_value=0.14, max_value=16.20, key='creatinine_baseline1')
    sbp_variance_value1 = st.number_input("sbp variance value: systolic blood pressure(mmHg)", value=310.85, min_value=0.50, max_value=2550.72, key='sbp_variance_value1')
    ventilation1 = st.selectbox("ventilation(Yes or No)", options=[0, 1], format_func=lambda x: 'No' if x == 0 else 'Yes', key="ventilation1")
    pco21 = st.number_input("pco2:Lastest recorded (mmHg)", value=75.61, min_value=0.00, max_value=104.00, key='pco21')
    totalco21 = st.number_input("totalco2:The lastest recorded total of CO2(mEq/L)", value=23.76, min_value=4.00, max_value=51.00, key='totalco21')
    weight1 = st.number_input("weight:The lastest recorded weight(kg)", value=83.18, min_value=28.80, max_value=190.00, key='weight1')
    gcs1 = st.number_input("gcs: Glasgow Coma Scale(No unit)", value=12.15, min_value=3.00, max_value=15.00, key='gcs1')
